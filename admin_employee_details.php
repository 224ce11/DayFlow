...
// Fetch Stats
// 1. Leave Stats
$l_approved_q = $conn->query("SELECT count(*) as c FROM leave_requests WHERE user_id = $mapped_emp_id AND status = 'Approved'");
$total_approved_leaves = $l_approved_q->fetch_assoc()['c'];

$l_pending_q = $conn->query("SELECT count(*) as c FROM leave_requests WHERE user_id = $mapped_emp_id AND status = 'Pending'");
$total_pending_leaves = $l_pending_q->fetch_assoc()['c'];

// 2. Attendance Stats for current month
$curr_month = date('Y-m');
...
// 3. Yearly Presence
$curr_year = date('Y');
$yearly_att = $conn->query("SELECT count(*) as c FROM attendance WHERE user_id = $mapped_emp_id AND status = 'Present' AND year(date) = '$curr_year'");
$presents_this_year = $yearly_att->fetch_assoc()['c'];
            ...
            <div style="display: flex; flex-direction: column; gap: 32px;">
                ...
                <div class="card" style="padding: 32px; border-radius: var(--radius-lg); background: var(--card-bg); box-shadow: var(--shadow-md);">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                        <h3><i class="fa fa-chart-pie" style="color: var(--primary); margin-right: 10px;"></i>Monthly & All-time Stats</h3>
...
-
                    <div class="stat-card-grid">
                        <div class="mini-stat-card">
                            ...
                            <div class="mini-stat-label">Month Present</div>
                        </div>
                        <div class="mini-stat-card">
                            <div class="mini-stat-val" style="color: var(--error);"><?php echo $stats['Absent']; ?></div>
                            <div class="mini-stat-label">Month Absent</div>
                        </div>
                        <div class="mini-stat-card">
                            <div class="mini-stat-val" style="color: var(--primary);"><?php echo $total_approved_leaves; ?></div>
                            <div class="mini-stat-label">Leaves Approved</div>
                        </div>
                        <div class="mini-stat-card">
                            <div class="mini-stat-val" style="color: var(--warning);"><?php echo $total_pending_leaves; ?></div>
                            <div class="mini-stat-label">Leaves Pending</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card" style="padding: 32px; border-radius: var(--radius-lg); background: var(--card-bg); box-shadow: var(--shadow-md);">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        <div style="padding: 20px; background: var(--surface-50); border-radius: var(--radius-md); border: 1px solid var(--surface-200);">
                            <div style="font-size: 14px; color: var(--text-light); margin-bottom: 4px;">Monthly Salary</div>
                            <div style="font-size: 28px; font-weight: 700; color: var(--text-dark);">₹<?php echo number_format($user['salary'], 2); ?></div>
                        </div>
                        <div style="padding: 20px; background: var(--surface-50); border-radius: var(--radius-md); border: 1px solid var(--surface-200);">
                             <div style="font-size: 14px; color: var(--text-light); margin-bottom: 4px;">Job Type</div>
                             <div style="font-size: 20px; font-weight: 600; color: var(--primary); margin-top: 6px;">Full-time</div>
                        </div>
                    </div>
                </div>
